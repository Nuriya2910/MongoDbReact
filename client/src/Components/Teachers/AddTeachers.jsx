...
import Alert2 from "../AlertsTeacher/Alert2"


export default () => {
    const [aler, setAler] = useState(false)
    const [aler2, setAler2] = useState(false)
    ...
    const checkInp = async () => {
        if (inp1.current.value === "" || inp2.current.value === "" || inp3.current.value === "" || inp4.current.value === "" || inp5.current.value === "" || inp6.current.value === "") {
            ...
            let res = await axios.post(`${config.backApi}/teachers`, user, axiosConfig)
            setAler2(true)
            setTimeout(() => {
                setAler2(false)
            }, 3000);
    ...
    return (
        <>
            ...
            <div className="container pt-5">
                <h1 className="mb-5">Add teachers</h1>
